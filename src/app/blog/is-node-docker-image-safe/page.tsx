...
const SEVERITY_ROWS: { tag: string; total: number; buckets: { label: string; count: number }[] }[] = [
  {
    ...
    total: 30726,
    buckets: [
      { label: "Critical", count: 1794 },
      { label: "High", count: 8867 },
      { label: "Medium", count: 17133 },
      { label: "Low", count: 1058 },
    ],
  },
  {
    tag: "node:22-alpine",
    total: 306,
    buckets: [
      { label: "Critical", count: 23 },
      { label: "High", count: 141 },
      { label: "Medium", count: 134 },
      { label: "Low", count: 8 },
    ],
  },
];
        ...
        <section className="grid gap-3">
          ...
          <figure className="overflow-x-auto surface-card p-4">
            <svg
              ...
              role="img"
              aria-label="Grouped bar chart of ScanRook severity counts. node:22: 30,726 findings total, 1,794 critical, 8,867 high, 17,133 medium, 1,058 low. node:22-alpine: 306 findings total, 23 critical, 141 high, 134 medium, 8 low."
            >
              <title>Grouped bar chart of ScanRook severity counts. node:22: 30,726 findings total, 1,794 critical, 8,867 high, 17,133 medium, 1,058 low. node:22-alpine: 306 findings total, 23 critical, 141 high, 134 medium, 8 low.</title>
            ...
            <figcaption className="text-xs text-gray-500 dark:text-gray-400 mt-2">
              The same scan data as a chart &mdash; ScanRook v1.14.2 warm-cache scan of node:22 and
              node:22-alpine, 2026-07-04. Bar length is proportional to the count in each severity bucket,
